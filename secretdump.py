...
class DumpSecrets:
        ...
        try:
            ...
            if self.__useKeyListMethod is True:
                try:
                    ...
                    logging.error(f'Something went wrong with the Kerberos Key List approach.: {str(e)}')
                ...
                if self.__justDC is False and self.__justDCNTLM is False and self.__canProcessSAMLSA:
                    ...
                    try:
                        self._extracted_from_dump_75(bootKey)
                    except Exception as e:
                        if logging.getLogger().level == logging.DEBUG:
                            import traceback
                            traceback.print_exc()
                        logging.error(f'LSA hashes extraction failed: {str(e)}')
                if self.__isRemote:
            ...
            if logging.getLogger().level == logging.DEBUG:
                ...
                traceback.print_exc()
            logging.error(e)
            if self.__NTDSHashes is not None and isinstance(e, KeyboardInterrupt):
                while True:
    ...
    # TODO Rename this here and in `dump`
    def _extracted_from_dump_75(self, bootKey):
...
if __name__ == '__main__':
    ...
    group.add_argument('-dc-ip', action='store', metavar="ip address",
                       help='IP Address of the domain controller. If ommited it use the domain part (FQDN) specified '
